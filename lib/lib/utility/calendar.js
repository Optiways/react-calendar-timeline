...
function setUnitValue(value, unit, callback) {
  ...
  switch (unit) {
    case 'year':
      date.setYear(callback(date.getFullYear()));
      break;

    case 'month':
      date.setMonth(callback(date.getMonth()));
      break;

    case 'week': // ???

    case 'day':
      date.setDate(callback(date.getDate()));
      break;

    case 'hour':
      date.setHours(callback(date.getHours()));
      break;

    case 'minute':
      date.setMinutes(callback(date.getMinutes()));
      break;

    case 'seconds':
      date.setSeconds(callback(date.getSeconds()));
      break;
  }
...
function iterateTimes(start, end, unit, timeSteps, callback) {
  var time = setUnitStart(start, unit);

  if (timeSteps[unit] && timeSteps[unit] > 1) {
    time = setUnitValue(time, unit, function (value) {
      return value - value % timeSteps[unit];
    });
  }

  while (time.valueOf() < end) {
    var nextTime = setUnitValue(new Date(time), unit, function (value) {
      return value + (timeSteps[unit] || 1);
    });
