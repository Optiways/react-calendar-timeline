  ...
  _createClass(Columns, [{
    ...
    value: function render() {
      ...
      (0, _calendar.iterateTimes)(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, function (time, nextTime) {
        ...
        switch (minUnit) {
          case 'year':
            minUnitValue = time.getFullYear();
            break;

          case 'month':
            minUnitValue = time.getMonth();
            break;

          case 'week': // ???

          case 'day':
            minUnitValue = time.getDate();
            break;

          case 'hour':
            minUnitValue = time.getHours();
            break;

          case 'minute':
            minUnitValue = time.getMinutes();
            break;

          case 'seconds':
            minUnitValue = time.getSeconds();
            break;
        }
        ...
        if (verticalLineClassNamesForTime) {
          classNamesForTime = verticalLineClassNamesForTime(time, nextTime - 1);
        } // TODO: rename or remove class that has reference to vertical-line


        var classNames = 'rct-vl' + (firstOfType ? ' rct-vl-first' : '') + (minUnit === 'day' || minUnit === 'hour' || minUnit === 'minute' ? " rct-day-".concat(time.getDate(), " ") : ' ') + classNamesForTime.join(' ');
